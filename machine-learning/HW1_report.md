...
- 차원의 저주라는 것이 있어서, 차원이 높을수록 데이터가 희박해져서 분석이 어려워진다
...
- 왜? PCA는 전체 데이터가 넓게 퍼진 방향만 찾았지, 두 클래스를 나누는 데 좋은 방향을 찾은 게 아니니까요.

LDA 투영 (아래 그래프):
- 파란색과 빨간색이 명확하게 분리되어 있습니다.
- 이 방향으로 투영하면 두 클래스를 쉽게 구분할 수 있습니다.
- 왜? LDA는 처음부터 두 클래스를 가장 잘 나누는 방향을 목표로 찾았기 때문입니다.
...
여기서 한 가지 확인해본 것이 정보보존율입니다. 1024차원을 2차원으로 줄이면 정보가 얼마나 남는지가 궁금했습니다.
...
이것을 보고 1024개의 숫자 중 상당수는 사실 중복된 정보라는 것을 실감했습니다.
...
왜 그런지 한참 찾아봤더니:
- Sw는 1024x1024 크기의 행렬인데, 역행렬을 구하려면 full rank여야 합니다
...
과제 지시문에도 정보보존율 95%로 PCA를 우선 수행하라고 되어 있었는데, 그 이유가 바로 이것이었습니다. PCA로 1024차원을 61차원으로 먼저 줄이면, Sw가 61x61 크기가 되므로 역행렬을 구할 수 있게 됩니다.
